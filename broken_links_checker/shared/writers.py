...
def write_error(directory=FILE_DIR, error='', exception=None):
    ...
    with open(os.path.join(directory, 'error.txt'), 'a', encoding='utf-8') as f:
        f.write(errormsg)
...
def write_debug(directory=FILE_DIR, msg='', exception=None):
    ...
    with open(os.path.join(directory, 'debug.txt'), 'a', encoding='utf-8') as f:
        f.write(debugmsg)


def write_details(directory=FILE_DIR, title='', url=''):
    with open(os.path.join(directory, 'details.txt'), 'a', encoding='utf-8') as f:
        f.write(title + '\r\n')
        f.write(url + '\r\n')
