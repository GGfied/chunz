...
def convert_to(folder, source, timeout=None):
    args = [libreoffice_exec(), '-env:UserInstallation=file://{}'.format(FILE_DIR), '--headless', '--convert-to', 'pdf:writer_pdf_Export', '--outdir', folder, source]
...
def setup():
    args = [libreoffice_exec(), '-env:UserInstallation=file://{}'.format(FILE_DIR), '--headless', '--invisible', '--terminate_after_init']
